...
enum MethodApi { delete, get, path, post, put }
...
class ApiUtils {
  ...
  Future<Result<MapJson, BackError>> makeRequest({
    ...
    final dio.Options options = dio.Options(headers: allHeaders);

    try {
      switch (method) {
        case MethodApi.delete:
          response = await _client.deleteUri(uri, options: options);
        case MethodApi.get:
          response = await _client.getUri(uri, options: options);
        case MethodApi.path:
          response = await _client.patchUri(uri, data: data, options: options);
        case MethodApi.post:
          response = await _client.postUri(uri, data: data, options: options);
        case MethodApi.put:
          response = await _client.putUri(uri, data: data, options: options);
      }
