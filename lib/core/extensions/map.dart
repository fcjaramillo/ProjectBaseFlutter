...
extension MapExtension on Map<String, dynamic> {
  ...
  String getString(String key, {String defaultValue = '', bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  int getInt(String key, {int defaultValue = 0, bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  num getNum(String key, {num defaultValue = 0, bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  bool getBool(String key, {bool defaultValue = false, bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  Map<String, T> getMapType<T>(
    String key, {
    Map<String, T> defaultValue = const <String, Never>{},
    bool alert = false,
  }) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
      ...
      _customAssert(
        containsKey(key) && this[key] is! Map<String, T>,
        "The key $key isn't a Map<String, dynamic>",
      );
    }
    if (this[key] is! Map) {
    ...
    final Map<String, dynamic> map = this[key];
    if (map.values.every((dynamic value) => value is T)) {
      final Map<String, T> newMap = <String, T>{};
      map.forEach((String k, dynamic v) => newMap[k] = v as T);
      return newMap;
    }
    return defaultValue;
  }
  ...
  bool? getBoolOr(String key, {bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  List<T>? getListOr<T>(String key, {bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  Map<String, dynamic>? getMapOr(String key, {bool alert = false}) {
    if (alert) {
      _customAssert(!containsKey(key), "The key $key, doesn't exist");
  ...
  List<T> getModelList<T>(
    ...
    if (alert) {
      _customAssert(!containsKey(value), "The key $value, doesn't exist");
...
extension AllMaps on Map<dynamic, dynamic> {
