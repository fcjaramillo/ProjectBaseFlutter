...
description: This skill is for Flutter interface design — dashboards, admin panels, apps, tools, and interactive products. NOT for marketing design (landing pages, marketing sites, campaigns).
---

# Interface Design (Flutter)

Build Flutter interface design with craft and consistency. Uses ThemeData, Material Design 3, and widget composition.

## Scope

**Use for:** Dashboards, admin panels, SaaS apps, tools, settings pages, data interfaces.

**Not for:** Landing pages, marketing sites, campaigns. Redirect those to a frontend/marketing design skill.
...
**Token names feel like implementation detail.** But your ThemeData, ColorScheme, and ThemeExtension values are design decisions. `ink` and `parchment` evoke a world. `neutral700` and `surface2` evoke a template. Someone reading only your theme definitions should be able to guess what product this is.
...
- **The token test:** Read your ThemeData, ColorScheme, and ions out loud. Do they sound like they belong to this product's world, or could they belong to any project?
...
**Key decisions (Flutter):**
- **Sidebars:** Use `colorScheme.surface` or `colorScheme.surfaceContainerLowest` for both sidebar and canvas. Use `BorderSide` with low opacity for separation.
- **Dropdowns:** Use `Material` with `elevation: 2` or `surfaceContainerHigh` above cards. Avoid same elevation as parent.
- **Inputs:** Use `colorScheme.surfaceContainerHighest` or slightly darker than surface. `Decoration` with `color` signals "type here" without heavy borders.

### Borders

Borders should disappear when you're not looking for them, but be findable when you need structure. Use `Color.withOpacity(0.05)` to `Color.withOpacity(0.12)` — blends with background. Solid `Colors.grey` borders look harsh.

Build a progression — not all borders are equal. Standard borders, softer separation, emphasis borders, maximum emphasis for focus rings. Match intensity to the importance of the boundary. Use `BoxDecoration.border` with `BorderSide`.
...
**Every time** you write Flutter UI code — even small additions — state:

```
Intent: [who is this human, what must they do, how should it feel]
Palette: [ColorScheme / AppColors — and WHY they fit this product's world]
Depth: [BoxDecoration borders / BoxShadow / elevation — and WHY this fits the intent]
Surfaces: [colorScheme.surface* — and WHY this color temperature]
Typography: [TextTheme — and WHY it fits the intent]
Spacing: [Layout.space* / BasePadding — and WHY this base unit]
```
...
## Token Architecture (ThemeData / ColorScheme)

Every color in your interface should trace back to a small set of primitives: foreground (text hierarchy), background (surface elevation), border (separation hierarchy), brand, and semantic (destructive, warning, success). Use `ThemeData`, `ColorScheme`, and `ThemeExtension` — no random `Color(0xFF...)` values.

### Text Hierarchy

Don't just have "text" and "gray text." Build four levels — primary, secondary, tertiary, muted. Use `textTheme` with `displayLarge`, `bodyLarge`, `bodyMedium`, `labelSmall` and opacity variants. Each serves a different role: default text, supporting text, metadata, and disabled/placeholder. Use all four consistently. If you're only using two, your hierarchy is too flat.

### Border Progression

Borders aren't binary. Build a scale that matches intensity to importance — standard separation, softer separation, emphasis, maximum emphasis. Use `BorderSide` with varying opacity. Not every boundary deserves the same weight.

### Control Tokens

Form controls have specific needs. Don't reuse surface colors — use `InputDecorationTheme`, `FilledButtonThemeData`, `OutlinedButtonThemeData`. Create dedicated `ThemeExtension` for control backgrounds, control borders, and focus states. This lets you tune interactive elements independently from layout surfaces.

## Spacing

Pick a base unit (4px or 8px) and stick to multiples. Use `Layout` or similar ions in `lib/ui/ions/`. Build a scale: `spaceXS`, `spaceS`, `spaceM`, `spaceL`, `spaceXL`. Use `EdgeInsets.all()`, `SizedBox`, `Gap`. Random values signal no system.

## Padding

Keep it symmetrical. Use `EdgeInsets.symmetric()` or `EdgeInsets.all()`. If one side has a value, others should match unless content naturally requires asymmetry.

## Depth

Choose ONE approach and commit:
- **Borders-only** — Clean, technical. Use `BoxDecoration` with `border` only. For dense tools.
- **Subtle shadows** — Soft lift. Use `BoxShadow` with low blur. For approachable products.
- **Layered shadows** — Premium, dimensional. Use multiple `BoxShadow` in `BoxDecoration`. For cards that need presence.
- **Surface color shifts** — Use `ColorScheme.surfaceContainerLowest` through `surfaceContainerHigh`. Background tints establish hierarchy without shadows.

Don't mix approaches.

## Border Radius

Sharper feels technical. Rounder feels friendly. Build a scale in ions — `BorderRadius.circular(4)` for inputs, `8` for cards, `12` for modals. Don't mix sharp and soft randomly.

## Typography

Build distinct levels with `TextTheme`. Headlines need `FontWeight.w600` and tight `letterSpacing` for presence. Body needs comfortable weight for readability. Labels need `FontWeight.w500` at smaller sizes. Data needs `fontFamily` monospace and `fontFeatures: [FontFeature.tabularFigures()]`. Don't rely on size alone — combine size, weight, and letter-spacing.

## Card Layouts

A metric card doesn't have to look like a plan card doesn't have to look like a settings card. Design each card's internal structure for its specific content — but keep the surface treatment consistent: same `BoxDecoration`, border weight, shadow depth, corner radius, padding scale.

## Controls

Use Flutter widgets with `ThemeData` overrides. For `DropdownButton` and `showDatePicker`, wrap with `Theme` or use `DropdownButtonFormField` / custom `InkWell` + `showModalBottomSheet` for full control. Build custom components when needed — `PopupMenuButton`, `showDatePicker`, styled `Material` containers.

## Iconography

Icons clarify, not decorate — if removing an icon loses no meaning, remove it. Choose one icon set (`MaterialIcons`, `Icons`, `LucideIcons`, etc.) and stick with it. Give standalone icons presence with `Container` or `CircleAvatar` with subtle background.

## Animation

Fast micro-interactions, smooth easing. Use `Duration(milliseconds: 150)` for hover/focus. Larger transitions use `200–250ms`. Use `Curves.easeOut` or `CurvedAnimation`. Avoid `Curves.elasticOut` in professional interfaces.

## States

Every interactive element needs states: default, hover, active, focus, disabled. Use `MaterialState` variants in `ButtonStyle`, `InputDecoration`. Data needs states too: loading, empty, error. Missing states feel broken.

## Navigation Context

Screens need grounding. A data table floating in space feels like a component demo, not a product. Include `NavigationBar`, `NavigationRail`, `Drawer`, or `Scaffold` with `AppBar`. Show location indicators, breadcrumbs, active nav state. Use same `colorScheme.surface` for sidebar and content with `Border` separation.

## Dark Mode

Dark interfaces have different needs. Shadows are less visible on dark backgrounds — lean on borders for definition. Semantic colors (success, warning, error) often need slight desaturation. Use `ThemeData.dark()` and `ColorScheme.dark()`. The hierarchy system still applies, just with inverted values.
...
Read `.interface-design/system.md` (or `docs/design_system.md`) and apply. Decisions are made.
...
If yes, write to `.interface-design/system.md` or `docs/design_system.md`:
- Direction and feel
...
Design tokens and UI constants belong in:
- **Theme:** `lib/ui/theme/` — `ThemeData`, `ColorScheme`, `AppColors`
- **Ions:** `lib/ui/ions/` — spacing (`Layout`), padding (`BasePadding`), radius, typography, shadows
- **Widgets:** `lib/ui/widgets/` — atoms, molecules, organisms (Atomic Design)
...
- `references/principles.md` — Flutter code examples, ThemeData, BoxDecoration, dark mode
- `references/validation.md` — Memory management, when to update system.md
- `references/critique.md` — Post-build craft critique protocol
- `references/example.md` — Subtle layering in Flutter

# Commands

- `/interface-design:status` — Current system state
- `/interface-design:audit` — Check code against system (ThemeData, ions, widgets)
- `/interface-design:extract` — Extract patterns from Flutter widgets
